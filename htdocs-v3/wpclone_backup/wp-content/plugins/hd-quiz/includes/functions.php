...
function hdq_save_settings()
{
    ...
    $hdq_nonce = sanitize_text_field($_POST['nonce']);
    if (!wp_verify_nonce($hdq_nonce, 'hdq_settings_nonce')) {
...
function hdq_get_results($quiz_settings)
{
    ...
    $result_text = sanitize_text_field(get_option("hd_results")); // in case of translation
    ...
    $results_percent = sanitize_text_field(get_option("hd_qu_percent")); ?>
...
function hdq_print_quiz_in_loop()
{
    ...
    echo '<a href = "'.$permalink.'" rel="noamphtml" class = "hdq_quiz_start hdq_button" role = "button" title = "'.$label.'">'.$label.'</a>';
}
...
function hdq_print_jPaginate($hdq_id)
{
    ...
    echo '<div class = "hdq_jPaginate"><div class = "hdq_next_button hdq_jPaginate_button hdq_button" data-id = "' . $hdq_id . '">' . $next_text . '</div></div>';
}
...
function hdq_print_finish($hdq_id, $jPaginate)
{
    ...
    echo '<div class = "hdq_finish"><div class = "hdq_finsh_button hdq_button '.$jPaginate.'" data-id = "' . $hdq_id . '">' . $finish_text . '</div></div>';
}
