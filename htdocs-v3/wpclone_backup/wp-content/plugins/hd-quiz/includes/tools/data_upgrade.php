...
<div id="main" style="max-width: 800px; background: #f3f3f3; border: 1px solid #ddd; margin-top: 2rem">
    ...
    <p>HD Quiz has grown considerably in features and complexity over the years making this page necessary.</p>
	...
	<center><div data-quizzes = "<?php echo $quizzes; ?>" data-questions = "<?php echo $questions; ?>" id = "hdq_tool_update_data_start" class = "hdq_button" role = "button" title = "Start update">
		BEGIN UPDATE
	</div></center>
...
function hdq_tool_upgrade_quiz_data()
{
    ...
    if (!empty($tax_terms) && !is_wp_error($tax_terms)) {
        foreach ($tax_terms as $tax_terms) {
            $quiz_id = $tax_terms->term_id;
            $term_meta = get_option("taxonomy_term_$quiz_id");
            if (isset($term_meta) && $term_meta != null && $term_meta != "" || 5 != 6) {
                $q = array();
