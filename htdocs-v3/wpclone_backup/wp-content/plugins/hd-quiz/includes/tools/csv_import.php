...
<div id="main" style="max-width: 800px; background: #f3f3f3; border: 1px solid #ddd; margin-top: 2rem">
    ...
    <center>
        ...
        <form action="<?php echo get_admin_url(null, "?page=hdq_tools_csv_importer"); ?>" method="post"
            ...
            <div
                style="display: grid; grid-template-columns: 1fr max-content; grid-gap: 2em; align-items: center; width: 100%; max-width: 600px">

                <div>
                    <input
                        style="width: 0.1px; height: 0.1px; opacity: 0; overflow: hidden; position: absolute; z-index: -1;"
                        type="file" accept=".csv" name="hdq_csv_file_upload" id="hdq_csv_file_upload" required="">
                    <label class="hdq_button2"
                        style="width: 100%; border-radius: 3px; font-weight: 400; text-overflow: ellipsis; white-space: nowrap; cursor: pointer; display: inline-block; overflow: hidden; padding: 0.625rem 1.25rem"
                        for="hdq_csv_file_upload">
                        <span class="dashicons dashicons-upload"></span> <span class="hdq_file_label">Choose a
                            file…</span>
                    </label>
                </div>
                <input type="submit" class="hdq_button" id="hdq_start_csv_upload" title="Begin Import"
                    value="BEGIN IMPORT" />
            </div>
        </form>
    </center>
...
function hdq_parse_csv_data($csvFile = "", $nonce = "")
{
    if (!current_user_can('edit_others_pages')) {
        echo 'permission not granted';
        die();
    ...
    for ($i = $start; $i < ($start + $atonce) && $i < count($csvAsArray); $i++) {
        if ($csvAsArray[$i][0] != "" && $csvAsArray[$i][1] != "") {
            $quizID = term_exists($csvAsArray[$i][0], "quiz");
            if ($quizID == null) {
                // create new quiz
                $quizID = wp_insert_term(
                    $csvAsArray[$i][0], // the term
                    'quiz' // the taxonomy
                );
            ...
            $fields["title"]["value"] = $csvAsArray[$i][1];
            $fields["title"]["type"] = "title";
            $fields["selected"]["value"] = array($csvAsArray[$i][12]);
            ...
            $fields["extra_text"]["value"] = "";
            $fields["extra_text"]["type"] = "editor";
            $fields["featured_image"]["value"] = 0;
            $fields["featured_image"]["type"] = "image";
            
            $answers = array();
            array_push($answers, array("answer" => $csvAsArray[$i][2], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][3], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][4], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][5], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][6], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][7], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][8], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][9], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][10], "image" => 0));
            array_push($answers, array("answer" => $csvAsArray[$i][11], "image" => 0));
    ...
    if ($start === 0) {
        $atonce = intval($atonce);
        echo '<script>setTimeout(function(){HDQ.parseNext("'.$csvFile.'", '.$atonce.', '.count($csvAsArray).');}, 1000);</script>';
    }
