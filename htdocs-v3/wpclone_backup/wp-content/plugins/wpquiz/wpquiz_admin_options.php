...
if (!current_user_can('manage_options'))  {
	...
	if (($editno > 0) && ($delno == 0))
	{
		...
		{		
			...
			$store_results = 0;
			$store_results_value = htmlspecialchars($_POST['wpquiz_store_results']);
			...
			if (($questioncheck == "") || ($answercheck == "") || ($correctanswercheck == "") || ($quiztitle == "")) {
				...
				do {
					...
					$questioncheck = htmlspecialchars($_POST['wpquiz_question' . $i]);
					$answercheck = htmlspecialchars($_POST['wpquiz_correctA' . $i]);					
					$correctanswercheck = htmlspecialchars($_POST['wpquiz_correctSA' . $i]);
		...
		if($_POST['bravo'] == 'x1') {  
			...
			if (($questioncheck == "") || ($answercheck == "") || ($correctanswercheck == "") || ($quiztitle == "")) {
				...
				do {
					...
					$answercheck = htmlspecialchars($_POST['wpquiz_correctA' . $i]);					
					$correctanswercheck = htmlspecialchars($_POST['wpquiz_correctSA' . $i]);
					// bug +1 ?$correctanswercheck = htmlspecialchars($_POST['wpquiz_correctSA' . $i] + 1);
...
function wpquiz_output_form($quizid = 0 )
{
	...
	// load up quiz for editing
	
		$sql = "SELECT `id`, `quiztitle`, `store_results` FROM " . $tbl_wpquiz_quiz. " WHERE id = $quizid";
		$quizdb = $wpdb->get_row($sql);
		$quiz_name = stripslashes($quizdb->quiztitle);
		$store_results = stripslashes($quizdb->store_results);
		
		$sql = "SELECT `id`, `question`, `answers`, `correctAnswer` FROM " . $tbl_wpquiz_question. " WHERE `quizid` = $quizid ORDER BY id ASC";
		$quizes = $wpdb->get_results ( $sql );
		...
		if (($wpdb->num_rows > 0) && (!is_null($quizes))) {
			...
			foreach ($quizes as $q ){				
				...
				$i++;
				$lq = stripslashes($q->question);
				$la = stripslashes($q->answers);
				...
				{					
					...
					$c++;
					if ($lan == $q->correctAnswer)
					{
						echo '<option value="' . $lan .'" selected = "selected">' . $lan . '</option>';
					}
					else
					{
