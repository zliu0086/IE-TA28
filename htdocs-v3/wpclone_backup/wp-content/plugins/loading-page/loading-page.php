...
if(!function_exists('loading_page_admin_resources')){
    function loading_page_admin_resources($hook){
        if(strpos($hook, "loading-page") !== false){
            ...
            wp_enqueue_script('lp-admin-script', LOADING_PAGE_PLUGIN_URL.'/js/loading-page-admin.js', array('jquery', 'thickbox', 'farbtastic'), 'free-1.0.70');
        }
    ...
    function loading_page_enqueue_scripts()
	{
        ...
        if( $loadingScreen )
		{
			...
			wp_enqueue_script('jquery');
			 wp_enqueue_style('codepeople-loading-page-style', LOADING_PAGE_PLUGIN_URL.'/css/loading-page.css', array(), 'free-1.0.70', false);
			wp_enqueue_style('codepeople-loading-page-style-effect', LOADING_PAGE_PLUGIN_URL.'/css/loading-page'.(($op['pageEffect'] != 'none') ? '-'.$op['pageEffect'] : '').'.css', array(), 'free-1.0.70', false);
            ...
            if($s)
			{
                if(!empty($s['style']))
				{
                    wp_enqueue_style('codepeople-loading-page-style-'.$s['id'], $s['style'], array(), 'free-1.0.70', false);
                }

                if(!empty($s['script']))
				{
                    wp_enqueue_script('codepeople-loading-page-script-'.$s['id'], $s['script'], array('jquery'), 'free-1.0.70', false);
                    $required[] = 'codepeople-loading-page-script-'.$s['id'];
                }
            }
            wp_enqueue_script('codepeople-loading-page-script', LOADING_PAGE_PLUGIN_URL.'/js/loading-page.js', $required, 'free-1.0.70', false);
            ...
            <form method="post">
				...
				<div class="postbox">
					...
					<div class="inside lp-video-tutorial <?php echo $loading_page_video_tutorial_status; ?>" style="position:relative;">
						...
						<iframe src="https://www.youtube.com/embed/5x_LtjoCFUY" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
						</div>
					</div>
				</div>
                <div class="postbox">
                    ...
                    <div class="inside">
                        ...
                        <table class="form-table">
							...
							<tr>
                                ...
                                <td><input type="checkbox" name="lp_remove_in_on_load" <?php echo((!empty($loading_page_options['remove_in_on_load'])) ? 'CHECKED' : '' ); ?> /> <i><?php _e('Hides the loading screen in the onload event of window - or - as soon as possible',LOADING_PAGE_TD); ?></i></td>
                            </tr>
							<tr>
                            ...
                            <tr>
                                ...
                                <td>
									...
									<input type="text" name="lp_loading_screen_display_in_pages" value="<?php if( !empty( $loading_page_options['lp_loading_screen_display_in_pages'] ) ) print $loading_page_options['lp_loading_screen_display_in_pages']; ?>"> <i><?php _e('Type one, or more post/pages IDs, comma separated ","', LOADING_PAGE_TD); ?></i></div>
							...
							<tr>
								...
								<td><input type="text" name="lp_loading_screen_exclude_from_pages" value="<?php if( !empty( $loading_page_options['lp_loading_screen_exclude_from_pages'] ) ) print esc_attr($loading_page_options['lp_loading_screen_exclude_from_pages']); ?>"> <i><?php _e('Type one, or more post/pages IDs, comma separated ","', LOADING_PAGE_TD); ?></i></td>
							</tr>
							<tr>
							...
							<tr>
								<th colspan="2">
									<?php _e( 'Exclude the loading screen if Elementor Maintenance or Coming Soon modes are enabled' ); ?>
									<input type="checkbox" name="lp_loading_screen_exclude_from_elementor_maintenance" <?php echo((!empty($loading_page_options['lp_loading_screen_exclude_from_elementor_maintenance'])) ? 'CHECKED' : '' ); ?> />
								</th>
							</tr>
							<tr><td colspan="2"><hr /></td></tr>
                            <tr>
						...
						<div style="border: 1px solid #DADADA; padding:10px;">
							...
							<table class="form-table">
								<tr>
									...
									<td>
										<input type="checkbox" name="lp_deactivateDeepSearch" <?php echo ((empty($loading_page_options['deepSearch'])) ? 'CHECKED' : ''); ?>/> <i><?php _e( 'If the loading screen stops in some percentage, tick the checkbox', LOADING_PAGE_TD ); ?></i>
									</td>
								</tr>
								<tr>
									<th><?php _e( 'The pages\' background is visible', LOADING_PAGE_TD ); ?></th>
									<td>
										<input type="checkbox" name="lp_modifyDisplayRule" <?php echo ((!empty($loading_page_options['modifyDisplayRule'])) ? 'CHECKED' : ''); ?>/> <i><?php _e( 'If website\'s background is visible before the loading screen, tick the checkbox', LOADING_PAGE_TD ); ?></i>
									</td>
								</tr>
                ...
                <div class="postbox">
                    ...
                    <div class="inside">
                        ...
                        <p><img src="<?php print(LOADING_PAGE_PLUGIN_URL.'/images/consumption_graph.png'); ?>" style="max-width:100%;" /></p>
                        <table class="form-table">
                            <tr>
							...
							<tr>
								...
								<td><input type="text" DISABLED /> <i><?php _e('Type one, or more post/pages IDs, comma separated ","', LOADING_PAGE_TD); ?></i></td>
							</tr>
                        </table>
						<div style="border: 1px solid #DADADA; padding:10px;">
