...
class Question_Categories_List_Table extends WP_List_Table{
     ...
     * @return mixed
     */
    public static function get_question_categories( $per_page = 20, $page_number = 1 ) {
    ...
    function column_items_count( $item ) {
        global $wpdb;
        $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}aysquiz_questions WHERE category_id = " . esc_sql( $item['id'] );
    ...
    public function prepare_items() {
        ...
        $this->items = self::get_question_categories( $per_page, $current_page );
    }
