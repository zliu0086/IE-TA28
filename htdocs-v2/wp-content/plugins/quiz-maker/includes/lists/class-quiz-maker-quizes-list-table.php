...
class Quizes_List_Table extends WP_List_Table{
    ...
    public function add_or_edit_quizes($data){
        ...
        if( isset($data["quiz_action"]) && wp_verify_nonce( $data["quiz_action"],'quiz_action' ) ){
            ...
            $fail_score_message = isset($data['ays_fail_score_message']) ? stripslashes($data['ays_fail_score_message']) : '<h4 style="text-align: center;">'. __("Oops!", $this->plugin_name) .'</h4><p style="text-align: center;">'. __("You are not passed the quiz! <br> Try again!", $this->plugin_name) .'</p>';
