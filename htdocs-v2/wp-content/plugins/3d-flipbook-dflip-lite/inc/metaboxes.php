...
class DFlip_Meta_boxes {
        ...
        <div class="dflip-tabs normal-tabs">
          ...
          foreach ( (array)$tabs as $id => $title ) {
            ...
            <div id="dflip-tab-content-<?php echo $id ?>"
              ...
              <code>[dflip id="<?php echo $postId ?>"<?php echo( $active_set == true ? ' type="' . $id . '"' : '' ) ?>
                ][/dflip]</code>
              <?php $active_set = true; ?>
            </div>
          <?php } ?>
  ...
  public function save_meta_boxes( $post_id, $post ) {
    ...
    $raw_data       = $_POST['_dflip'];
    $sanitized_data = array();
    
    //Source Tab
    $sanitized_data['source_type'] = sanitize_text_field( $raw_data['source_type'] );
    $sanitized_data['pdf_source']  = esc_url_raw( $raw_data['pdf_source'] );
    $sanitized_data['pdf_thumb']   = esc_url_raw( $raw_data['pdf_thumb'] );
    
    $page_list_raw = $raw_data['pages'];
    $page_list     = array();
    if ( is_array( $page_list_raw ) ) {
      foreach ( (array)$page_list_raw as $page_key => $page_value ) {
        $page             = array();
    ...
    //Layout tab
    $sanitized_data['webgl']             = sanitize_text_field( $raw_data['webgl'] );
    $sanitized_data['hard']              = sanitize_text_field( $raw_data['hard'] );
    $sanitized_data['bg_color']          = sanitize_text_field( $raw_data['bg_color'] );
    $sanitized_data['bg_image']          = esc_url_raw( $raw_data['bg_image'] );
    $sanitized_data['duration']          = sanitize_text_field( $raw_data['duration'] );
    $sanitized_data['height']            = sanitize_text_field( $raw_data['height'] );
    $sanitized_data['texture_size']      = sanitize_text_field( $raw_data['texture_size'] );
    $sanitized_data['auto_sound']        = sanitize_text_field( $raw_data['auto_sound'] );
    $sanitized_data['enable_download']   = sanitize_text_field( $raw_data['enable_download'] );
    $sanitized_data['page_mode']         = sanitize_text_field( $raw_data['page_mode'] );
    $sanitized_data['single_page_mode']  = sanitize_text_field( $raw_data['single_page_mode'] );
    $sanitized_data['controls_position'] = sanitize_text_field( $raw_data['controls_position'] );
    $sanitized_data['direction']         = sanitize_text_field( $raw_data['direction'] );
    $sanitized_data['force_fit']         = sanitize_text_field( $raw_data['force_fit'] );
    $sanitized_data['autoplay']          = sanitize_text_field( $raw_data['autoplay'] );
    $sanitized_data['autoplay_duration'] = sanitize_text_field( $raw_data['autoplay_duration'] );
    $sanitized_data['autoplay_start']    = sanitize_text_field( $raw_data['autoplay_start'] );
    $sanitized_data['page_size']         = sanitize_text_field( $raw_data['page_size'] );
    
    //Outline/sidemenu tab
    $sanitized_data['auto_outline']      = sanitize_text_field( $raw_data['auto_outline'] );
    $sanitized_data['auto_thumbnail']    = sanitize_text_field( $raw_data['auto_thumbnail'] );
    $sanitized_data['overwrite_outline'] = sanitize_text_field( $raw_data['overwrite_outline'] );
    
    $sanitized_data['outline'] = isset( $raw_data['outline'] ) ? $this->array_text_sanitize( $raw_data['outline'] ) : array();
