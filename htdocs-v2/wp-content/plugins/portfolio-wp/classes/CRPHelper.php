...
private static $banners = null;
  ...
  {
      if (is_null(CRPHelper::$banners)) {
          $lastLoadedAt = get_option(CRP_BANNERS_LAST_LOADED_AT);
          $lastLoadedAt = !empty($lastLoadedAt) ? $lastLoadedAt : 0;
          if ($lastLoadedAt == 0 || time() - $lastLoadedAt > 1 * 24 * 60 * 60) {
              $result = CRPHelper::sendApiRequest(array(
                  'pkg' => CRP_LICENSE_TYPE,
                  'action' => 'get_banners'
              ), 'GET');
              $cache = array();
              if (!empty($result) && $result['rc'] == CRPHelper::API_RC_OK) {
                  $cache = !empty($result['banners']) ? $result['banners'] : array();
                  update_option(CRP_BANNERS_CONTENT, $cache);
                  update_option(CRP_BANNERS_LAST_LOADED_AT, time());
              }
          } else {
              $cache = get_option(CRP_BANNERS_CONTENT);
          }
          CRPHelper::$banners = $cache;
      }
      if (!empty(CRPHelper::$banners[$zone])) {
          return CRPHelper::$banners[$zone];
      }
      return array();
  }
  ...
  {
      $result = wp_remote_post( CRP_API_URL,
          array(
              'method' => $method,
              'timeout' => 45,
              'blocking' => true,
