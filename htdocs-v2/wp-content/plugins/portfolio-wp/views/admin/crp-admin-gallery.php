...
<div class="crp-portfolio-header">
    ...
    <div class="crp-three-parts crp-fl crp-title-part"><input id="crp-portfolio-title" class="crp-portfolio-title" name="portfolio-title" maxlength="250" placeholder="Enter gallery title" type="text"></div>
...
<div class="crp-empty-project-list-alert">
    <h3>You don't have items in this gallery yet!</h3>
</div>
...
<div class="crp-gallery-wrapper">
    <div class="crp-add-item-boxes">
        ...
        <div class="crp-add-item-box"><a id="crp-add-video-button" class='button-secondary crp-add-project-button crp-glazzed-btn crp-glazzed-btn-green gkit-tooltip' href='#' title='<?php echo htmlentities('<div class="crp-tooltip-content">Upgrade to Premium version for Local Videos</div>'); ?>'>+ Add video (PRO)</a></div>
        <div class="crp-add-item-box"><a id="crp-add-youtube-button" class='button-secondary crp-add-project-button crp-glazzed-btn crp-glazzed-btn-green gkit-tooltip' href='#' title='<?php echo htmlentities('<div class="crp-tooltip-content">Upgrade to Premium version for Youtube Videos</div>'); ?>'>+ Add youtube (PRO)</a></div>
        <div class="crp-add-item-box"><a id="crp-add-vimeo-button" class='button-secondary crp-add-project-button crp-glazzed-btn crp-glazzed-btn-green gkit-tooltip' href='#' title='<?php echo htmlentities('<div class="crp-tooltip-content">Upgrade to Premium version for Vimeo Videos</div>'); ?>'>+ Add vimeo (PRO)</a></div>
        <div class="crp-add-item-box"><a id="crp-add-iframe-button" class='button-secondary crp-add-project-button crp-glazzed-btn crp-glazzed-btn-green gkit-tooltip' href='#' title='<?php echo htmlentities('<div class="crp-tooltip-content">Upgrade to Premium version for iFrames</div>'); ?>'>+ Add iframe (PRO)</a></div>
        <div class="crp-add-item-box"><a id="crp-add-map-button" class='button-secondary crp-add-project-button crp-glazzed-btn crp-glazzed-btn-green gkit-tooltip' href='#' title='<?php echo htmlentities('<div class="crp-tooltip-content">Upgrade to Premium version for Maps</div>'); ?>'>+ Add map (PRO)</a></div>
    </div>
...
function crp_addProjectItem(crp_project )
{
    ...
    html +=
        '<tr id="crp-gallery-project-' + crp_project.id + '" data-id="' + crp_project.id + '" class="crp-gallery-project">' +
            ...
            '<td class="crp-content">' +
                ...
                '<div class="crp-content-box"><textarea rows=3 placeholder="Enter description" name="project.description"></textarea></div>' +
            '</td>' +
    ...
    jQuery("textarea[name='project.description']", html).val(crp_project.description);
...
function crp_updateUI(){
    ...
    if(crp_portfolio.projects && crp_portfolio.corder){
        for(var crp_projectIndex = 0; crp_projectIndex < crp_portfolio.corder.length; crp_projectIndex++){
            ...
            cItem.description = CrpBase64.decode(cItem.description);
...
function crp_updateModel(){
    ...
    crp_portfolio.extoptions = {
        all_cats: {},
        type: '<?php echo CRPGridType::GALLERY; ?>'
    };
    ...
    jQuery(".crp-gallery-project").each(function(key, elem){
        ...
        crp_activeProject.description = CrpBase64.encode(jQuery("textarea[name='project.description']", elem).val());
...
function onPortfolioOptions() {
    if (crp_portfolio.isDraft) {
        alert("Save the draft gallery before changing the view options");
    } else {
