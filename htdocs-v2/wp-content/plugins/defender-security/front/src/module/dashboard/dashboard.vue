<template>
  <div class="sui-wrap" :class="maybeHighContrast()">
    <div class="defender-dashboard">
      ...
      <summary-box>
        ...
        <div class="sui-summary-segment">
          <ul class="sui-list">
            ...
            <li>
              <span class="sui-list-label">{{ __("Malware Scan Issues") }}</span>
      ...
      <div class="sui-row">
        <div class="sui-col-md-6">
          ...
          <waf v-if="is_free === 0 && queue_waf === true"></waf>
          <waf_free v-if="is_free===1"></waf_free>
        ...
        <div class="sui-col-md-6">
          ...
          <audit v-if="is_free===0 && queue_waf"></audit>
    ...
    <preset-config-modal v-else-if="show_features==true"></preset-config-modal>
  </div>
</template>
...
import TwoFa from "./component/two-fa";
import Waf from "./component/waf";
import waf_free from './component/waf-free'
import WafModal from "./component/waf-modal";
import PresetConfigModal from "./component/preset-config-modal";
import PresetConfig from "./component/preset-config";
...
export default {
  ...
  data: function () {
    return {
      ...
      queue_waf: dashboard.waf.waf.maybe_show,
      scan: {
        count: 0,
